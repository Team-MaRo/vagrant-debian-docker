...
The CI workflow (`.github/workflows/ci.yml`) only lints/validates the sources;
no secrets are required. Run the same checks locally:

```shell
packer fmt -check -recursive src/
packer validate src/
shellcheck bin/*.sh src/*.sh .github/scripts/*.sh
```
...
A single box version can carry both architectures under the same `virtualbox`
provider, and Vagrant 2.4+ automatically serves each user the build matching
their host. Because VirtualBox can't emulate a foreign CPU, build each
architecture on a **native** host:

| Architecture | Build host |
| --- | --- |
| `amd64` | an x86-64 machine (e.g. a Windows PC) |
| `arm64` | an Apple Silicon Mac (needs the Apple-Silicon build of VirtualBox 7.1+) |

On **each** host, install VirtualBox + Packer + Vagrant, then build and publish
...
`publish.sh` auto-detects the host architecture (`vagrant cloud publish
--architecture`) and adds it to the shared version. `amd64` is marked as the
provider's default architecture; override with `VAGRANT_CLOUD_DEFAULT_ARCH=arm64`,
or force a specific arch with `VAGRANT_CLOUD_ARCH`. Order doesn't matter, but the
version string must match on both machines. If the registry refuses to add the
second architecture to an already-released version, stage both with
`VAGRANT_CLOUD_RELEASE=0` and release the version once in the HCP UI.
