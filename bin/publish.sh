...
#
# A sha256 checksum of the box is computed and attached on upload, so Vagrant
# verifies the download's integrity (the registry's per-architecture checksum).
#
...
# Compute a sha256 checksum so Vagrant can verify box integrity on download
# (populates the provider/architecture's checksum on the registry).
echo "Computing sha256 checksum of $FILE ..."
